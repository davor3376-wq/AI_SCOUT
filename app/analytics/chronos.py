...
class GifGenerator:
    ...
    def generate_timelapse(self, current_job_id: str, bbox: List[float], current_files: List[str]) -> Optional[str]:
        ...
        # Sort again by date label if possible, though previous sort on jobs was likely enough.
        # But current might be mixed in if we didn't rely on job list order.
        # Let's rely on the order we appended: Historical then Current.

        images = []
        try:
            for date_label, tif_path in image_files:
                with rasterio.open(tif_path) as src:
                    ...
                    colored[nan_mask] = [1, 1, 1, 1] # White
                    ...
                    # Add Text Overlay (Date) using PIL (if font available) or just assume context
                    # For simplicity, we skip text overlay or try basic drawing if needed.
                    # Given environment constraints, simple image is safer.
            ...
            images[0].save(
                ...
                duration=800, # 0.8s per frame
                loop=0
            )
            logger.info(f"Saved Timelapse to {output_path}")
            return output_path
